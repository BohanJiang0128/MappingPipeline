...
def get_closest_vertices_mask_from_ES(
    ...
    """
    # print(S.shape)
    embedding_resized = E[0].to(device)
    coarse_segm_resized = S[0].to(device)
    mask = coarse_segm_resized[0] > 0
    # print(mask.shape)
    closest_vertices = torch.zeros(mask.shape, dtype=torch.long, device=device)
    all_embeddings = embedding_resized[:, mask].t()
    ##Bohan Code
    # mesh_vertex_embeddings = mesh_vertex_embeddings[:6890]
    ##Bohan Code
    ...
    for chunk in range((len(all_embeddings) - 1) // size_chunk + 1):
        ...
        import numpy as np
        # print(np.max(chunk_embeddings.detach().cpu().numpy()))
        # print(np.min(chunk_embeddings.detach().cpu().numpy()))
        # print(np.max(mesh_vertex_embeddings.detach().cpu().numpy()))
        # print(np.min(mesh_vertex_embeddings.detach().cpu().numpy()))
        edm.append(
            torch.argmin(
